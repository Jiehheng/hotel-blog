<?php
	...
	if ($rs)
	{
		...
		$sql="insert into sys_account
		(serial_id,account,password,unit,name,identity_id,birthday,telephone,cellphone,email,fix_address,com_address,take_office_day,Contact_time,create_time,authority,job)
		values
		('{$serial_id}','{$account}','{$password}','{$unit}','{$name}','{$identity_id}','{$birthday}','{$telephone}','{$cellphone}','{$email}','{$fix_address}','{$com_address}','{$take_office_day}','{$Contact_time}',NOW(),'{$authority}','{$job}')";
		$rs1=$db->Execute($sql);
		if ($rs1)
		{
			//特休
