<?php
	...
	// 抓出 eip_new 中帳號的 serial_id 與 account
	...
	$sql="SELECT SQL_CALC_FOUND_ROWS * FROM sys_unit order by sort asc limit {$start},{$page_row}";
	$rs=$db->Execute($sql);
    if ($rs)
	{
		...
		if ($total_rows>0)
		{
			while ($rows=$db->fetch_assoc($rs))
			{
				...
				$list .= '<tr bgcolor="white" onMouseOver="this.className=\'lvtColDataHover\'" onMouseOut="this.className=\'lvtColData\'">
							...
							<td align="center">'.$depart_sid[$rows['depart_sid']].'</td>
