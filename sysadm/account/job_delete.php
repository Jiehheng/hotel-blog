<?php
	...
	{
		$sql="delete from sys_job where serial_id='{$del_whichs[$i]}'";
		$rs=$db->Execute($sql);
		if (!$rs)
			die('資料庫存取發生異常');
	}
	echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></head><body>
			<script type="text/javascript">alert("刪除成功"); window.location.href="index.php?action=action&which=job_list"</script>
			</body></html>';
	exit;
?>	