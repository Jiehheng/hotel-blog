<?php
	...
	$sql="SELECT SQL_CALC_FOUND_ROWS * from case_district order by sort asc limit {$start},{$page_row}";
	$rs=$db->Execute($sql);
    if ($rs)
	{
		...
		if ($total_rows>0)
		{
			while ($rows=$db->fetch_assoc($rs))
			{
				...
				$list .= '<tr bgcolor="white" onMouseOver="this.className=\'lvtColDataHover\'" onMouseOut="this.className=\'lvtColData\'">
							<td align="center" width="60"><input type="checkbox" value="'.$rows['serial_id'].'" '.$edit.'></td>
			...
			$list .= '<tr bgcolor="white" onMouseOver="this.className=\'lvtColDataHover\'" onMouseOut="this.className=\'lvtColData\'">
				<td colspan="5" align="center">無任何資料</td>
			</tr>';
		}
	}else
		die('資料庫發生錯誤');	
	$xtpl->assign('district_list', $list);
	$xtpl->parse('table');
