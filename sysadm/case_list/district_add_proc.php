<?php
	...
	$sql="insert into case_district (serial_id,caption,stid,sort) values ('{$serial_id}','{$caption}','{$stid}','{$sort}') ";
	$rs=$db->Execute($sql);

	if ($rs)
	{
		echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></head><body>
			<script type="text/javascript">alert("新增成功"); window.location.href="index.php?action=action&which=district_list"</script>
			</body></html>';
		exit;
	}else
		die('資料庫發生錯誤');
