<?php
	...
	$sql="insert into `sys_bulletin_type` (serial_id,caption,sort) values ('{$serial_id}','{$caption}','{$sort}') ";
	$rs=$db->Execute($sql);
	if ($rs)
	{
		echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></head><body>
			<script type="text/javascript">alert("新增成功"); window.location.href="index.php?action=action&which=type_list"</script>
			</body></html>';
		exit;
	}else
		die('資料庫發生錯誤');
