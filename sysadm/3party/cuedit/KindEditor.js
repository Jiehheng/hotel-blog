...
var KE_IMAGE_ATTACH_PATH = "/trades/member/";
var KE_IMAGE_UPLOAD_CGI = "/trades/upfile.php";
...
var KE_LANG = {
	INPUT_URL		: "請輸入正確的URL位址。",
	SELECT_IMAGE	: "請選擇圖片。",
	INVALID_IMAGE	: "只能選擇GIF,JPG,PNG,BMP格式的圖片，請重新選擇。",
	INVALID_FLASH	: "只能選擇SWF格式的文件，請重新選擇。",
	INVALID_MEDIA	: "只能選擇MP3,WAV,WMA,WMV,MID,AVI,MPG,ASF,RAR格式的文件，請重新選擇。",
	INVALID_REAL	: "只能選擇RM,RMVB格式的文件，請重新選擇。",
...
var KE_TOP_TOOLBAR_ICON = Array(
	...
	Array('KE_UNDO', 'undo.gif', '返回'),
	Array('KE_REDO', 'redo.gif', '前進'),
	...
	Array('KE_JUSTIFYFULL', 'justifyfull.gif', '兩邊對齊'),
...
var KE_POPUP_MENU_TABLE = Array(
	...
	"KE_IMAGE", "KE_MYFILES", "KE_FLASH", "KE_MEDIA", "KE_REAL", "KE_LINK"
);
...
function KindCheckMediaFileType(cmd, file, separator)
{
	...
	if (cmd == 'KE_REAL') {
		...
		if (fileExt != 'mp3' && fileExt != 'wav' && fileExt != 'wma' && fileExt != 'wmv' && fileExt != 'mid' && fileExt != 'avi' && fileExt != 'mpg' && fileExt != 'asf' && fileExt != 'rar') {
...
function KindPopupMenu(cmd)
{
	switch (cmd)
	{
		...
		case 'KE_MYFILES':
			var str = '';
			str += '<div id="POPUP_'+cmd+'" style="width:250px;'+KindGetMenuCommonStyle()+'">';
			str += '<iframe name="KindImageP8MYFILESIframe" id="KindImageP8MYFILESIframe" frameborder="0" style="width:250px;height:85px;padding:0;margin:0;border:0;">';
			str += '</iframe></div>';
			return str;
			break;
		case 'KE_FLASH':
	...
	switch (cmd)
	{
		case 'KE_IMAGE':
			...
			if (KE_UPLOAD_MODE == true) {
				...
				'<input type="file" name="fileData" id="imgFile" size="14" style="border:1px solid #555555;" onclick="javascript:document.getElementById(\'imgLink\').value=\'http://\';"  onpropertychange="javascript:parent.KindImagePreview(); " /><input type=hidden size=4 value="3" name="utype" /><input type=hidden size=4 value="" name="imgwidthl" id="imgwidthl"/><input type=hidden size=4 value="" name="imgheightl" id="imgheightl" />';
			} else {
		...
		case 'KE_MYFILES':
			...
			str += '<table cellpadding="0" cellspacing="0" style="width:100%;font-size:12px;">' + 
				'<tr><td style="width:50px;padding:5px;">URL</td>' +
				'<td style="width:200px;padding-top:5px;padding-bottom:5px;"><input type="text" id="hyperLink" value="http://" style="width:190px;border:1px solid #555555;background-color:#FFFFFF;"></td>' +
				'<tr><td style="padding:5px;">'+KE_LANG['SUBJECT']+'</td>' +
				'<td style="padding-bottom:5px;"><input type="text" id="FilesSay" style="width:190px;border:1px solid #555555;" /></td></tr>' + 
				'<tr><td colspan="2" style="padding-bottom:5px;" align="center">' +
				'<input type="submit" name="button" id="'+cmd+'submitButton" value="'+KE_LANG['CONFIRM']+'" onclick="javascript:parent.KindDrawMYFILESEnd();" style="border:1px solid #555555;background-color:'+KE_BUTTON_COLOR+';" /> ' +
				'<input type="button" name="button" value="'+KE_LANG['CANCEL']+'" onclick="javascript:parent.KindDisableMenu();" style="border:1px solid #555555;background-color:'+KE_BUTTON_COLOR+';" /></td></tr>';
			str += '</table>';
...
function KindImagePreview()
{
	...
	KE_IMAGE_DOCUMENT.getElementById('imgWidthl').value = width;
	KE_IMAGE_DOCUMENT.getElementById('imgHeightl').value = height;
...
function KindDrawMYFILESEnd()
{
	var url = KE_MYFILES_DOCUMENT.getElementById('hyperLink').value;
	var uri = KE_MYFILES_DOCUMENT.getElementById('FilesSay').value;
	var ks = new KindString();
	ks.append('<a href="', url);
	ks.append('"><img src="', '../images/file.gif');
	ks.append('" width="', '20');
	ks.append('" height="', '20');
	ks.append('" border="', '0');
	ks.append('" title="', uri);
	ks.append('" align="absmiddle" /><font style="font-size:9.5pt; color:#0099FF">', uri);
	ks.append('</font></a>');
	KindSelect();
	KindInsertHtml(ks.toString());
	KindDisableMenu();
...
function KindDisableToolbar(flag)
{
	if (flag == true) {
		...
		for (i = 0; i < KE_TOOLBAR_ICON.length; i++) {
			...
			if (KE_TOOLBAR_ICON[i][0] == 'KE_SOURCE' || KE_TOOLBAR_ICON[i][0] == 'KE_PREVIEW') {
				continue;
			}
