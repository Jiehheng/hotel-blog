<?php
	...
	{
		$sql_ins = " WHERE `identity_id` != ''";
	}
	
	///帳號列表
	$start = ($now_page-1)*$page_row; //資料分頁
	$sql = "SELECT SQL_CALC_FOUND_ROWS * from sys_members{$sql_ins} limit {$start},{$page_row}";
	$rs = $db->Execute($sql);
    if ($rs)
	{
		...
		if ($total_rows>0)
		{
			...
			while ($rows=$db->fetch_assoc($rs))
			{
				if($rows['identity_id'])
				{
					if(!$rows['serviceis']) $want2join = '<span style="color:red;">《等候申請仲介商》</span>';
					else $want2join = '<span style="color:blue;">《仲介商》</span>';
				}
				else $want2join = '<span style="color:gray;">《一般會員》</span>';
				...
				$list .= '<tr bgcolor="white" onMouseOver="this.className=\'lvtColDataHover\'" onMouseOut="this.className=\'lvtColData\'">
							...
							<td align="left">'.$want2join.$rows['account'].'</td>
