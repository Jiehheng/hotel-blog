<?php
	...
	$sql="update `sys_depart_mb` set `caption`='{$caption}',`stid`='{$stid}',`sort`='{$sort}' where `serial_id`='{$serial_id}'";
	$rs=$db->Execute($sql);
	if ($db->num_rows($rs)>=0)
	{
		echo '<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"></head><body>
			<script type="text/javascript">alert("更新成功"); window.location.href="index.php?action=action&which=depart_list"</script>
			</body></html>';
		exit;
	}else
		die('資料庫發生錯誤');
