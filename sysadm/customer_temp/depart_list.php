...
$sql = "SELECT SQL_CALC_FOUND_ROWS * from `sys_depart_mb` order by `sort` asc limit {$start},{$page_row}";
$rs = $db->Execute($sql);
if ($rs)
{
	...
	if ($now_page==1) $first_last='';
	else $first_last='<a href="javascript:;" onclick="change_page(\'1\')">第一頁</a>&nbsp;<a href="javascript:;" onclick="change_page(\''.($now_page-1).'\')">上一頁</a>&nbsp;';
	if ($now_page==$total_page) $next_final='';
	else $next_final='<a href="javascript:;" onclick="change_page(\''.($now_page+1).'\')">下一頁</a>&nbsp;<a href="javascript:;" onclick="change_page(\''.$total_page.'\')">最末頁</a>&nbsp;';
	...
	if ($total_rows>0)
	{
		...
		$list .= '<tr>
				...
				<td class="lvtCol" align="right" colspan="4">'.$first_last.$next_final.'&nbsp;資料筆數：'.$total_rows.'&nbsp;&nbsp;&nbsp;頁數：'.$now_page.'/'.$total_page.'</td>
			  </tr>';	
	}
	else
	{
		$list .= '<tr bgcolor="white" onMouseOver="this.className=\'lvtColDataHover\'" onMouseOut="this.className=\'lvtColData\'">
			<td colspan="3" align="center">無任何資料</td>
		</tr>';
	}
}
else die('資料庫發生錯誤');
$xtpl->assign('now_page', $now_page);
$xtpl->assign('depart_list', $list);
$xtpl->parse('table');
